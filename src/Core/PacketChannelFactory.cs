...
namespace System.Net.Mqtt
{
	internal class PacketChannelFactory : IPacketChannelFactory
	{
		...
		private IEnumerable<IFormatter> GetFormatters()
		{
			...
			formatters.Add (new IdentifiablePacketFormatter<PublishAck>(PacketType.PublishAck, id => new PublishAck(id)));
			formatters.Add (new IdentifiablePacketFormatter<PublishReceived>(PacketType.PublishReceived, id => new PublishReceived(id)));
			formatters.Add (new IdentifiablePacketFormatter<PublishRelease>(PacketType.PublishRelease, id => new PublishRelease(id)));
			formatters.Add (new IdentifiablePacketFormatter<PublishComplete>(PacketType.PublishComplete, id => new PublishComplete(id)));
			...
			formatters.Add (new IdentifiablePacketFormatter<UnsubscribeAck> (PacketType.UnsubscribeAck, id => new UnsubscribeAck(id)));
